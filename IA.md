...
O site é bilíngue (espanhol do Chile como padrão, português como alternativa), por causa da parceria COIL entre a UFSM e a Universidad Santo Tomás. Textos de interface vivem em `client/src/i18n/translations.js`, nunca hardcoded num componente. O conteúdo das perguntas da trilha vive nos módulos bilíngues de seed em `server/scripts/content`, porque a API envia esse catálogo ao client.
...
- [ ] Todo texto novo de interface visível ao usuário (inclusive `aria-label`/`alt`) veio de `client/src/i18n/translations.js`, existe em `es` e `pt`, e não gerou warning de tradução faltando no console
